...
{
  "postmortem": "A clear, professional 2-3 sentence explanation of what happened structurally and why the price hit target or stop-loss.",
...
Do not add markdown formatting or backticks around the JSON. Return only the raw JSON.`;
    ...
    const systemPrompt = `You are a professional trading postmortem analyzer. Return only valid raw JSON matching the requested schema. Everything must be in standard English.`;
    ...
    if (!result) {
      return NextResponse.json({
        data: {
          postmortem: "AI analysis is currently unavailable. The trade hit target under normal market conditions.",
    ...
    try {
      ...
      console.error("Failed to parse postmortem output:", result.content, parseError);
      return NextResponse.json({
        data: {
          postmortem: "Failed to format AI postmortem analysis. Check entry coordinates.",
          lesson: "Verify entry invalidation zones on your chart."
        }
      });
    }
  } catch (err) {
    console.error("Trade Postmortem API Route Error:", err);
    return NextResponse.json({ data: { error: "Failed to generate trade postmortem." } }, { status: 500 });
  }
}

...
export function PageShell({
  ...
  return (
    <section className={`min-h-screen ${ready ? "transition-colors duration-300" : ""}`}>
      <MobileNav open={mobileNavOpen} onClose={() => setMobileNavOpen(false)} />

      {/* Header */}
      <header className={`sticky top-0 z-20 border-b border-[var(--card-border)] bg-[var(--background)]/85 px-4 py-3.5 backdrop-blur-xl md:px-8 ${ready ? "transition-colors duration-300" : ""}`}>
        <div className="w-full flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3 shrink-0">
            ...
            <button
              ...
              className="flex h-9 w-9 items-center justify-center rounded-xl border border-[var(--card-border)] bg-[var(--card)] shadow-sm transition-colors hover:border-slate-300 dark:hover:border-slate-600 lg:hidden cursor-pointer"
            >
            ...
            <Link href="/" className="group flex items-center gap-2.5">
              <div className="flex h-9 w-9 items-center justify-center rounded-xl border-2 border-[var(--accent)] bg-[var(--accent-soft)] text-sm font-black text-[var(--accent)] transition-transform duration-300 group-hover:scale-105">
                FX
              </div>
              <div className="hidden sm:block">
                <p className="text-lg font-black tracking-tight leading-none text-slate-900 dark:text-white">FX</p>
                <p className="font-mono text-[9px] uppercase tracking-[0.16em] text-slate-500 mt-1">Market intelligence</p>
              </div>
            </Link>
          </div>

          {/* Middle section: Sleek Horizontal Navigation (Hidden on mobile) */}
          <nav className="hidden lg:flex items-center gap-1 border-l border-[var(--card-border)] pl-4 ml-2" aria-label="Horizontal navigation">
            {navItems.map((item) => {
              const active = pathname === item.href;
              return (
                <Link
                  key={item.href}
                  href={item.href}
                  className={`group relative flex items-center gap-2 rounded-xl px-3.5 py-1.5 text-sm font-extrabold transition-all duration-200 ${
                    active
                      ? "bg-[var(--accent-soft)] text-[var(--accent)]"
                      : "text-slate-600 hover:bg-slate-100/60 hover:text-slate-900 dark:text-slate-400 dark:hover:bg-white/5 dark:hover:text-slate-200"
                  }`}
          ...
          {/* Right section: Title/Label context indicator (middle-right) + theme toggle & action buttons */}
          <div className="flex items-center gap-3 ml-auto">
            {/* Title / Label text shown if space permits (hidden on narrow screens) */}
            <div className="hidden md:block text-right pr-3 border-r border-[var(--card-border)]">
              <p className="text-[9px] font-bold uppercase tracking-wider text-slate-400 font-mono leading-none">{label}</p>
              <h1 className="mt-1 text-xs font-extrabold tracking-tight leading-none text-slate-700 dark:text-slate-200">{title}</h1>
            </div>
            ...
            >
              {darkMode ? (
                ...
                <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-slate-600">
            ...
            {action ? (
              actionHref ? (
                <Link
                  href={actionHref}
                  className="rounded-xl bg-[var(--accent)] px-3.5 py-2 text-xs font-bold text-white shadow-sm transition-all duration-200 hover:-translate-y-0.5 hover:shadow-md cursor-pointer block text-center"
                >
                  {action}
                </Link>
              ) : (
                <button
                  ...
                  className="rounded-xl bg-[var(--accent)] px-3.5 py-2 text-xs font-bold text-white shadow-sm transition-all duration-200 hover:-translate-y-0.5 hover:shadow-md cursor-pointer"
                >
                  {action}
                </button>
              )
